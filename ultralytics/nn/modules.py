...
class DAM(nn.Module):
    ...
    def __init__(self, c1, c2, k=5, n=num_DAM()):
        ...
        # Create a list to hold the dynamic number of pooling layers
        self.pool_layers = nn.ModuleList([nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2) for _ in range(n - 1)])
    ...
    def forward(self, x):
        ...
        # Apply the dynamic number of pooling layers
        for pool_layer in self.pool_layers:
            y.append(pool_layer(y[-1]))
